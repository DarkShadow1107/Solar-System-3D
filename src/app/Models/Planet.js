define([
...
], function (Constants, CelestialObject, Orbit, ThreeText, RadialRingGeometry) {
	"use strict";
	...
	class Planet extends CelestialObject {
		...
		buildFullObject3D() {
			...
			this._orbitCentroid.add(this._threeObject, this._core, this._orbitLine.orbit, this._objectCentroid);
		...
		createGeometry(surface, atmosphere) {
			...
			var mesh = new THREE.Mesh(new THREE.SphereGeometry(this._threeRadius - 0.1, segmentsOffset, segmentsOffset));
			mesh.add(surface);
		...
		createSurface(base, topo, specular) {
			...
			var mesh = new THREE.Mesh(new THREE.SphereGeometry(this._threeRadius, segmentsOffset, segmentsOffset), surface);
		...
		createRingGeometry(data) {
			...
			var phiSegments = 80;
			var geometry = new RadialRingGeometry(innerRadius, outerRadius, thetaSegments);
		...
		createHighlight(amplitude) {
			...
			for (var i = 0; i <= resolution; i++) {
				...
				orbitLine.vertices.push(
					new THREE.Vector3(Math.cos(segment) * orbitAmplitude, Math.sin(segment) * orbitAmplitude, 0)
				);
			}
